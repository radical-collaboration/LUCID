...
class Pipeline:
    ...
    def register_pipeline_tasks(self):
        ...
        s1_task_desc = {'ranks': 1,
                        'named_env': 'rp',
                        'gpus_per_rank': 1,
                        'pre_exec': self.cfg_exec['task_pre_exec'] or [],
                        'environment': self.cfg_exec.get('task_environment', {})}

        s2_task_desc = {'ranks': 1,
                        'named_env': 'rp',
                        'pre_exec': self.cfg_exec['task_pre_exec'] or [],
                        'environment': self.cfg_exec.get('task_environment', {})}

        @self.flow.executable_task
        async def stage_1(image_path: str,
                          task_description: dict = s1_task_desc):  # noqa: B006
            """Segmentation."""
            ...
            return (f'python $RP_PILOT_SANDBOX/{run_script} '
                    + f'--image_path {image_path} '
                    + ' '.join([str(a) for a in arguments]))

        @self.flow.executable_task
        async def stage_2(task_description: dict = s2_task_desc):  # noqa: B006
            """Analysis."""
        ...
        setattr(self, 'stage_1', stage_1)
        setattr(self, 'stage_2', stage_2)

    async def run(self):
        """Main execution logic"""

        self.logger.info(f'Pipeline {self.name} started '
                         f'with {len(self.images)} images')
...
async def main():
    ...
    # adjust runtime
    run_description = dict(**config['run_description'],
                           input_staging=glob.glob(f'{SRC_DIR}/*'),
                           sandbox=os.path.abspath(args.work_dir or WFMS_DIR))
    if args.runtime:
        run_description['runtime'] = int(args.runtime)

    backend = await RadicalExecutionBackend(run_description)
    ...
    for pipe_id, input_dir in enumerate(input_dirs):
        ...
        p_task = p.run()
        pipes.append(p_task)
        print(f'{datetime_now()} Processing {p.name} has started')
